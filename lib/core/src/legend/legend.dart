...
class _SfLegendState extends State<SfLegend> {
  ...
  Widget? _buildLegend() {
    ...
    final double scrollbarThickness = defaultTargetPlatform == TargetPlatform.iOS ||
    ...
    if (widget.title == null) {
      if (widget.overflowMode == LegendOverflowMode.scroll) {
        ...
        current = ScrollConfiguration(
          ...
          child: SingleChildScrollView(
              scrollDirection:
                  widget.scrollDirection == Axis.horizontal ? Axis.horizontal : Axis.vertical,
            ...
            Flexible(
              child: widget.overflowMode == LegendOverflowMode.scroll
                  ? SingleChildScrollView(
                      scrollDirection: widget.scrollDirection ?? Axis.vertical, child: current)
...
class _LegendItemState extends State<_LegendItem> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    final Color? begin =
        widget.shader == null && widget.imageProvider == null ? widget.iconColor : null;
  ...
  String _getTrimmedText(String currentText, String? nextText, int index, int length) {
    ...
    if (_direction == Axis.horizontal &&
      ...
      if (index == length - 1) {
        ...
        refCurrentTextWidth =
            isLastInsideItem ? _getTextWidth(currentText) : _getTextWidth(currentText) / 2;
      } else {
      ...
      _isOverlapSegmentText =
          refCurrentTextWidth + refNextTextWidth > barSize.width + widget.itemSpacing!;
...
String _getTrimText(
    String text, TextStyle style, double maxWidth, TextPainter painter, double width,
...
class __SolidBarLegendItemState extends State<_SolidBarLegendItem> {
  ...
  Offset _getHorizontalTextOffset(int index, String text, int dataSourceLength) {
    ...
    if (widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
      final double width =
          _textDirection == TextDirection.rtl && widget.segmentSize!.width < _textPainter.width
              ? _textPainter.width
  ...
  Offset _getVerticalTextOffset(int index, String text, int dataSourceLength) {
    ...
    if (widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
      ...
      return Offset(
          0.0, widget.segmentSize!.width - _textPainter.height / 2 + widget.itemSpacing! / 2);
    } else {
    ...
    if (widget.index == widget.pointerController!.segmentIndex &&
      ...
      if (widget.direction == Axis.horizontal) {
        matrix4 = Matrix4.identity()
          ..translate(widget.pointerController!.position!.dx * widget.segmentSize!.width -
        ...
        matrix4 = Matrix4.identity()
          ..translate(
              0.0,
              widget.pointerController!.position!.dy * widget.segmentSize!.width -
                  (widget.pointerSize!.width / 2));
        current = Transform(transform: matrix4, child: current);
      }
    } else {
...
class _GradientBarLegendState extends State<_GradientBarLegend> {
  ...
  void _updateSegmentSize(double shortestSide) {
    if (_direction == Axis.horizontal) {
      final double availableWidth =
          widget.padding != null ? shortestSide - widget.padding!.horizontal : shortestSide;
    ...
    final double availableHeight =
        widget.padding != null ? shortestSide - widget.padding!.vertical : shortestSide;
    _segmentSize = widget.segmentSize == null
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (widget.items![0].text[0] == '{' &&
          ...
          final int positionIndex =
              widget.labelsPlacement == LegendLabelsPlacement.onItem ? i + 1 : i;
          if (widget.labelsPlacement == LegendLabelsPlacement.onItem) {
          ...
          _labels.add(_GradientBarLabel(
              text, _getTextOffset(text, positionIndex, length - 1, slab), _isOverlapSegmentText));
        }
  ...
  void _collectRageColorMapperLabels(int i, LegendItem item, String text, double slab, int length) {
        ...
        if (widget.labelOverflow == LegendLabelOverflow.ellipsis) {
          if (widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
            ...
            startText = _getTrimText(
                startText, widget.textStyle!, slab, _textPainter, textWidth, refNextTextWidth);
          }
        }
      }

      _labels.add(_GradientBarLabel(
          startText, _getTextOffset(startText, i, length, slab), _isOverlapSegmentText));
    } else if (i < length - 1) {
      text = _getTrimmedText(text, i, length, slab, widget.items![i + 1].text);
    }
    ...
    _labels.add(
        _GradientBarLabel(text, _getTextOffset(text, i + 1, length, slab), _isOverlapSegmentText));
  }
      ...
      if (widget.labelOverflow == LegendLabelOverflow.ellipsis && _isOverlapSegmentText) {
        if (widget.labelsPlacement == LegendLabelsPlacement.betweenItems) {
          ...
          return _getTrimText(currentText, widget.textStyle!, slab, _textPainter, textWidth,
              refNextTextWidth, isLastInsideItem);
        }
      }
  ...
  Widget _buildPointer(ThemeData themeData) {
    ...
    if (widget.pointerController!.position != null) {
      ...
      if (_direction == Axis.horizontal) {
        matrix4 = Matrix4.identity()
          ..translate(widget.pointerController!.position!.dx * _segmentSize.width -
              (widget.pointerSize!.width / 2));
        if (_isRTL) {
          matrix4.invert();
        }
  ...
  Widget build(BuildContext context) {
    ...
    _textPainter = TextPainter(
        textDirection: TextDirection.ltr, textScaler: MediaQuery.of(context).textScaler);
    _direction = widget.direction ??
        (widget.position == LegendPosition.top || widget.position == LegendPosition.bottom
            ? Axis.horizontal
            : Axis.vertical);
    textDirection =
        _isRTL ? (_direction == Axis.vertical ? TextDirection.ltr : textDirection) : textDirection;

...
Path _processStepLineShape(
  ...
  if (paint != null) {
    ...
    canvas.drawPath(
        isDashArray!
            ? _processDashPath(path, dashArray: _CircularIntervalList<double>(<double>[3, 2]))
            : path,
...
Path _processPieShape(
    ...
    Paint? paint,
    Paint? borderPaint}) {
  rect = Rect.fromLTWH(rect.left, rect.top + 1, rect.width - 1, rect.height - 1);
  final double x = rect.left + rect.width / 2;
  final double y = rect.top + rect.height / 2;
  final double width = rect.width;
  final double height = rect.height;
  ...
  path.moveTo(x, y);
  path.lineTo(x + r, y);
  path.arcTo(Rect.fromCircle(center: Offset(x, y), radius: r), _degreesToRadians(0),
      _degreesToRadians(270), false);
  path.close();
  path.moveTo(x + width / 10, y - height / 10);
  path.lineTo(x + r, y - height / 10);
  path.arcTo(Rect.fromCircle(center: Offset(x + 1, y - 1), radius: r), _degreesToRadians(0),
      _degreesToRadians(-90), false);
  path.close();
...
Path _processDoughnutShape(
  ...
  final bool hasBorder =
      borderPaint != null && borderPaint.color != Colors.transparent && borderPaint.strokeWidth > 0;
  if (isNeedToReturnPath) {
    if (hasBorder) {
      path1 = _getArcPath(path, radius / 4, radius / 2, Offset(x, y), 0, 270, 270, true);
    } else {
      path2 = _getArcPath(path, radius / 4, radius / 2, Offset(x + 1, y - 1), 0, -90, -90, true);
    }
  ...
  if (hasBorder) {
    canvas.drawPath(path1, borderPaint..color = Colors.grey.shade300.withOpacity(0.5));
  }
  canvas.drawPath(path2, paint);
  if (hasBorder) {
    canvas.drawPath(path2, borderPaint..color = Colors.grey.shade300.withOpacity(0.5));
  }
...
Path _processRadialBarShape(
  ...
  if (isNeedToReturnPath) {
    if (hasBorder) {
      path1 = _getArcPath(
          path, (radius / 2) - 2, radius / 2, Offset(x, y), 0, 360 - 0.01, 360 - 0.01, true);
    } else {
      path2 = _getArcPath(
          path, (radius / 2) - 2, radius / 2, Offset(x, y), startAngle!, endAngle!, degree!, true);
    }
    return path;
  }
...
/// Get the path of arc
Path _getArcPath(Path path, double innerRadius, double radius, Offset center, double startAngle,
    double endAngle, double degree, bool isAnimate) {
  ...
  final Point<double> radiusStartPoint =
      Point<double>(radius * cos(startAngle) + center.dx, radius * sin(startAngle) + center.dy);
  ...
  if (isFullCircle) {
    ...
    path.arcTo(Rect.fromCircle(center: center, radius: radius), midpointAngle.toDouble(),
        endAngle - midpointAngle.toDouble(), true);
  } else {
  ...
  if (isFullCircle) {
    ...
    path.arcTo(Rect.fromCircle(center: center, radius: innerRadius), midpointAngle.toDouble(),
        startAngle - midpointAngle.toDouble(), true);
  } else {
...
Path _processLineShape(
  ...
  if (isNeedMarker! && overlayMarkerType != null) {
    final Rect pathRect =
        Rect.fromCenter(center: rect.center, width: rect.width / 1.5, height: rect.height / 1.5);
    canvas.drawPath(
        getShapesPath(rect: pathRect, shapeType: overlayMarkerType), Paint()..color = paint!.color);
  }
...
Path _processColumnShape(
  ...
  path.lineTo(left + (rect.width / padding) + (rect.width / (padding * 2)),
      top - (rect.height / 4) - (padding / 2));
  path.lineTo(
      left + (rect.width / padding) + (rect.width / (padding * 2)), top + (rect.height / 2));
  path.lineTo(
      left - (rect.width / padding) - (rect.width / (padding * 2)), top + (rect.height / 2));
  path.close();

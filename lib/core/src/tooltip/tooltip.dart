...
class TooltipRenderObject extends SingleChildRenderObjectWidget {
  ...
  TooltipRenderBox createRenderObject(BuildContext context) {
    _tooltipState.renderBox =
        TooltipRenderBox(_tooltipState, _tooltipAnimation, _animationController);
...
class TooltipRenderBox extends RenderShiftedBox {
  ...
  TooltipRenderBox(this._tooltipState, this._tooltipAnimation, this._animationController,
      [RenderBox? child])
      : super(child);
  ...
  bool hitTest(BoxHitTestResult result, {required Offset position}) {
    if (child == null || _tooltipRect == null) {
      ...
      return child!.hitTest(result,
          position: position -
              (_isOutOfBoundInTop
                  ? _tooltipRect!.topLeft + Offset(0, _templateArrowHeight + _tooltipRect!.height)
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (_tooltipState._show) {
      if (((_animationController.value == 0 || _tooltipState._showDuration == 0) &&
              _animationController.status != AnimationStatus.dismissed) &&
          _tooltipState.widget.onTooltipRender != null) {
        final TooltipRenderArgs tooltipRenderArgs = TooltipRenderArgs(
            _header, _stringValue, _x != null && _y != null ? Offset(_x!, _y!) : null);
  ...
  void _renderTemplateTooltipView(PaintingContext context, Offset offset) {
    ...
    Offset arrowLocation = Offset(
        _x! - _templateSize.width / 2, _isOutOfBoundInTop ? _y! : _y! - _templateArrowHeight);
  ...
  void _renderArrowAndTemplatePath(Canvas canvas, Offset location, Offset templateLocation) {
    ...
    final RRect rect = RRect.fromLTRBAndCorners(templateLocation.dx, templateLocation.dy,
        templateLocation.dx + _templateSize.width, templateLocation.dy + _templateSize.height,
  ...
  void _renderDefaultTooltipView(Canvas canvas) {
    ...
    if (_header!.isNotEmpty) {
      final TextStyle headerTextStyle =
          style.copyWith(color: style.color ?? _tooltip.labelColor, fontWeight: FontWeight.bold);
  ...
  Rect _calculateBackgroundRect(
    ...
    final Rect rect = Rect.fromLTWH(
        _x!, _y!, width + (2 * _markerSize) + widthPadding, height + headerTextHeight + 10);
    final Rect newRect = Rect.fromLTWH(
        _boundaryRect.left + 20, _boundaryRect.top, _boundaryRect.width - 40, _boundaryRect.height);
    final Rect leftRect = Rect.fromLTWH(_boundaryRect.left - 5, _boundaryRect.top - 20,
        newRect.left - (_boundaryRect.left - 5), _boundaryRect.height + 40);
    ...
    if (_y! > _pointerLength + rect.height && _y! > _boundaryRect.top) {
      ...
      _isTop = false;
      _xPos = _x! - (rect.width / 2);
      _yPos = ((_y! >= _boundaryRect.top ? _y! : _boundaryRect.top) + _pointerLength / 2) +
          (_inversePadding ?? 0);
      _nosePointX = rect.left;
  ...
  void _drawTooltipBackground(
    ...
    _drawTooltipPath(canvas, tooltipRect, rect, backgroundPath, isTop, isLeft, isRight, startX,
        endX, animationFactor, xPosition, yPosition);

    final TextStyle textStyle = _tooltip.textStyle.copyWith(
      color: _tooltip.textStyle.color?.withOpacity(_tooltip.opacity) ?? _tooltip.labelColor,
  ...
  void _drawTooltipPath(
    ...
    if (isRight) {
      ...
      backgroundPath.lineTo(
          rect.right, isTop ? (factor - _borderRadius) : (factor + _borderRadius));
    ...
    if (_tooltip.elevation > 0) {
      if (tooltipRect.width * animationFactor > tooltipRect.width * 0.85) {
        canvas.drawShadow(
            _arrowPath, _tooltip.shadowColor ?? fillPaint.color, _tooltip.elevation, true);
      }
      canvas.drawShadow(
          tooltipPath, _tooltip.shadowColor ?? fillPaint.color, _tooltip.elevation, true);
    }
  ...
  void _drawTooltipText(
      Canvas canvas, RRect tooltipRect, TextStyle textStyle, Size result, double animationFactor) {
    ...
    if (_header!.isNotEmpty) {
      final TextStyle headerTextStyle = _tooltip.textStyle.copyWith(
        color: textStyle.color?.withOpacity(_tooltip.opacity) ?? _tooltip.labelColor,
      ...
      _markerPointY = tooltipRect.top +
          ((_header!.isNotEmpty) ? headerResult.height + (padding * 2) + 6 : (padding * 1.7));
      final int maxLinesOfHeader = getMaxLinesContent(_header);
      _drawText(
          _tooltip,
          canvas,
          _header!,
          Offset((tooltipRect.left + tooltipRect.width / 2) - headerResult.width / 2,
  ...
  void _renderTooltipText(
      Canvas canvas, RRect tooltipRect, Size textSize, TextStyle textStyle, int maxLines) {
  ...
  Offset _getTextPosition(Size textSize, RRect tooltipRect, int maxLines) {
    ...
    if (_tooltipState._isRtl) {
      if (maxLines > 1 && !(markerSize > 0)) {
        ...
        textOffsetX =
            tooltipRect.right - tooltipRect.width / 2 - textSize.width / 2 - 2 * markerSize;
      }
    } else {
      textOffsetX =
          (tooltipRect.left + 2 * markerSize + tooltipRect.width / 2) - textSize.width / 2;
    }
    return Offset(
        textOffsetX,
        _header!.isNotEmpty
  ...
  void _drawMultiLineRtlText(
      SfTooltip tooltip, Canvas canvas, String text, Offset point, TextStyle style,
    ...
    final double rectEndPointBeforeMarker =
        _getMarkerPosition(totalTextSize ?? Size.zero, markerSize ?? 0.0, tooltipRect ?? RRect.zero)
                .dx -
            (2 * (markerSize ?? 0.0));
  ...
  void _drawText(
    ...
    if (kIsWeb) {
      if (_animationFactor < 0.5) {
        style = style.copyWith(color: style.color!.withOpacity(_animationFactor));
      } else if (_animationFactor <= 1) {
        style = style.copyWith(color: style.color!.withOpacity(tooltip.opacity));
      }
    }

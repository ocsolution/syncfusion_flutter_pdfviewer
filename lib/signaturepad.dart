...
class SfSignaturePadState extends State<SfSignaturePad> {
  ...
  Future<ui.Image> toImage({double pixelRatio = 1.0}) {
    final RenderObject? signatureRenderBox = context.findRenderObject();
    // ignore: avoid_as
    return (signatureRenderBox! as RenderSignaturePad).toImage(pixelRatio: pixelRatio);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final Color strokeColor = widget.strokeColor ?? (isDarkTheme ? Colors.white : Colors.black);
...
class _SfSignaturePadRenderObjectWidget extends LeafRenderObjectWidget {
  ...
  @override
  void updateRenderObject(BuildContext context, RenderSignaturePad renderObject) {
    renderObject
      ..minimumStrokeWidth = minimumStrokeWidth
      ..maximumStrokeWidth = maximumStrokeWidth
      ..backgroundColor = backgroundColor
...
class RenderSignaturePad extends RenderBox {
  ...
  @override
  void performLayout() {
    double width = constraints.hasBoundedWidth ? constraints.maxWidth : _kDefaultWidth;

    if (constraints.minWidth > _kDefaultWidth) {
      width = constraints.minWidth;
    }

    double height = constraints.hasBoundedHeight ? constraints.maxHeight : _kDefaultHeight;
  ...
  void handleEvent(PointerEvent event, BoxHitTestEntry entry) {
    ...
    } else if (event is PointerUpEvent && _touchGestureFinder == event.pointer) {
  ...
  void _update(Offset touchOffset) {
    ...
    final _TouchPoint point = _TouchPoint(x: x, y: y, time: DateTime.now().millisecondsSinceEpoch);
    final List<_TouchPoint> lastPoints = _data[_data.length - 1];
    final double distance =
        lastPoints.isNotEmpty ? _distance(lastPoints[lastPoints.length - 1], point) : 1;
    if (distance > 0) {
  ...
  _Bezier? _calculateBezierPath(_TouchPoint point) {
    ...
    if (_lastPoints.length > 2) {
      ...
      final double velocity = _velocityFilterWeight * _velocity(startPoint, endPoint) +
          (1 - _velocityFilterWeight) * _lastVelocity;
      final double newWidth = max(_maximumStrokeWidth / (velocity + 1), _minimumStrokeWidth);

      final _Bezier curve =
          _Bezier.fromPoints(points: _lastPoints, start: _lastWidth, end: newWidth);
  ...
  void _drawSignatureCurve(_Bezier curve) {
    ...
    for (int i = 0; i < drawSteps; i++) {
      ...
      final double width = min(curve.startWidth + ttt * widthDelta, _maximumStrokeWidth);
  ...
  Future<ui.Image> toImage({double pixelRatio = 1.0}) async {
    // ignore: avoid_as
    return (layer! as OffsetLayer).toImage(Offset.zero & size, pixelRatio: pixelRatio);
  }
      ...
      if (points.length > 1) {
        for (int i = 0; i < points.length; i++) {
          ...
          final _TouchPoint point =
              _TouchPoint(x: basicPoint.x, y: basicPoint.y, time: basicPoint.time);
          if (i == 0) {
            _reset();
            _currentPath = Path();
            _pathCollection.add(_currentPath);
  ...
  void renderToContext2D(dynamic context2D) {
    ...
    if (!_restrictBezierPathCalculation) {
      for (int i = 0; i < _dotPoints.length; i++) {
        ...
        context2D.arc(
            point.dx, point.dy, (_minimumStrokeWidth + _maximumStrokeWidth) / 2, 0, pi * 2, true);
      }

      for (int i = 0; i < _bezierPoints.length; i++) {
        context2D.moveTo(_bezierPoints[i].x, _bezierPoints[i].y);
        context2D.arc(
            _bezierPoints[i].x, _bezierPoints[i].y, _bezierPoints[i].width / 2, 0, 2 * pi, false);
      }
      ...
      for (int i = 0; i < _data.length; i++) {
        if (_data[i].length == 1) {
          ...
          context2D.arc(
              point.x, point.y, (_minimumStrokeWidth + _maximumStrokeWidth) / 2, 0, pi * 2, true);
          context2D.fillStyle = 'rgba($strokePenColor)';
          context2D.fill();
        } else {
  ...
  void paint(PaintingContext context, Offset offset) {
    context.pushClipRect(needsCompositing, offset, Rect.fromLTWH(0, 0, size.width, size.height),
        (PaintingContext context, Offset offset) {
      final Canvas canvas = context.canvas;

      //Drawing the background of the SignaturePad
      canvas.drawRect(
          Rect.fromLTWH(offset.dx, offset.dy, size.width, size.height), _paintBackgroundStyle);
      ...
      if (_restrictBezierPathCalculation) {
        ...
        for (int i = 0; i < _data.length; i++) {
          if (_data[i].length == 1) {
            final _TouchPoint point = _data[i][0];
            canvas.drawCircle(Offset(point.x, point.y),
                (_minimumStrokeWidth + _maximumStrokeWidth) / 2, _paintStrokeStyle);
          } else {
        ...
        if (_dotPoints.isNotEmpty) {
          _paintStrokeStyle.strokeWidth = (_minimumStrokeWidth + _maximumStrokeWidth) / 2;
...
class _Bezier {
  _Bezier(
      this.startPoint, this.control2, this.control1, this.endPoint, this.startWidth, this.endWidth);
  ...
  static _Bezier fromPoints(
      {required List<_TouchPoint> points, required double start, required double end}) {
    final _TouchPoint c2 = calculateControlPoints(points[0], points[1], points[2])[1];
    final _TouchPoint c3 = calculateControlPoints(points[1], points[2], points[3])[0];
    return _Bezier(points[1], c2, c3, points[2], start, end);
  }

  static List<_TouchPoint> calculateControlPoints(_TouchPoint s1, _TouchPoint s2, _TouchPoint s3) {
    final double dx1 = s1.x - s2.x;
    final double dy1 = s1.y - s2.y;
    final double dx2 = s2.x - s3.x;
    final double dy2 = s2.y - s3.y;

    final Point<double> m1 = Point<double>((s1.x + s2.x) / 2.0, (s1.y + s2.y) / 2.0);
    final Point<double> m2 = Point<double>((s2.x + s3.x) / 2.0, (s2.y + s3.y) / 2.0);
  ...
  double length() {
    ...
    for (int i = 0; i <= steps; i += 1) {
      ...
      final double cx = point(t, startPoint.x, control1.x, control2.x, endPoint.x);
      final double cy = point(t, startPoint.y, control1.y, control2.y, endPoint.y);
